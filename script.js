...
function showPixelInfo(e) { /* 이전과 동일, 생략 */ }
...
// ▼▼▼ 핵심 수정: 이미지 클릭은 붉은 선만 그리고, 그래프는 건드리지 않음 ▼▼▼
previewCanvas.addEventListener('click', (e) => {
    ...
    if (y >= 0 && y < imageHeight) {
        selectedRowY = y;
        drawImage(); // Redraw image with the red line, but DOES NOT touch the profile graph.
    }
});
...
calculateAndPlotBtn.addEventListener('click', () => {
    ...
    // Step 값도 가져오지만, 그래프 계산에는 사용하지 않음 (저장용)
    const xStep = parseInt(cropXStep.value), yStep = parseInt(cropYStep.value);

    if ([xFrom, xTo, yFrom, yTo, xStep, yStep].some(isNaN)) return alert("모든 From, To, Step 값을 입력해주세요.");
    
    // 그래프용 데이터는 X축을 따라 Y축을 평균내는 방식
    plottedData = [];
    for (let x = xFrom; x < xTo; x++) {
        let ySum = 0;
        for (let y = yFrom; y < yTo; y++) {
            if (y * imageWidth + x < currentDisplayData.length) {
...
document.body.addEventListener('drop', async (e) => { if (e.dataTransfer.files.length > 0 && e.dataTransfer.files[0].name.toLowerCase().endsWith('.spe')) await parseSpeFile(e.dataTransfer.files[0]); });

// showPixelInfo 함수와 같이 길고 변하지 않는 부분은 간결하게 표시
...
function updateAnalysis() { if (!plottedData) { peakDeltaDisplay.textContent = "N/A"; peakCenterDisplay.textContent = "N/A"; integralValueDisplay.textContent = "N/A"; return; } if (peakLine1X !== -1 && peakLine2X !== -1) { peakDeltaDisplay.textContent = Math.abs(peakLine1X - peakLine2X); peakCenterDisplay.textContent = ((peakLine1X + peakLine2X) / 2).toFixed(2); } else { peakDeltaDisplay.textContent = "N/A"; peakCenterDisplay.textContent = "N/A"; } if (integralLine1X !== -1 && integralLine2X !== -1) { const start = Math.min(integralLine1X, integralLine2X), end = Math.max(integralLine1X, integralLine2X); let sum = 0; for (let i = start; i <= end; i++) sum += plottedData[i]; integralValueDisplay.textContent = sum.toExponential(3); } else { integralValueDisplay.textContent = "N/A"; } }
